  ...
  <v-container class="d-flex flex-column align-center">
      ...
      <v-form class="d-flex flex-column align-center" style="width: 100%">
        <div class="d-flex" style="width: 100%">
          <div style="width: 80%">
            ...
            <v-icon large class="my-auto pl-3 darkgrey--text" @click="swap()"> mdi-swap-vertical </v-icon>
      </div>
        
        <div class="d-flex" style="width: 80%">
          <v-switch v-model="depArr" :label="depArr ? 'Ankunft' : 'Abfahrt'"></v-switch>
      ...
      >
        ...
        ></v-date-picker>
          ...
          <v-btn elevation="5" @click="getTrip()" fab class="grey darken-3 white--text">
...
export default {
  ...
  methods: {
    async getStopList(depdes) {
      if(depdes == 'dep'){
        const { data } = await axios.get(`http://localhost:3000/points/${this.depInput}`);
      ...
      if(depdes == 'des'){
        const { data } = await axios.get(`http://localhost:3000/points/${this.desInput}`);
        if (data instanceof Array){
          this.desList = data;
        }
        else{
    ...
    async getTrip(){
      ...
      console.log(params);
      const { data } = await axios.get('http://localhost:3000/trip', {params,});
      console.log(data);
      this.trips = [];
